  ...
  return (
    <div className="mx-auto max-w-6xl grid gap-6">
      <Card className="bg-black/70 border-red-900/40">
        <CardHeader>
          <CardTitle className="text-red-50">Welcome back</CardTitle>
        ...
        <CardContent className="flex flex-wrap gap-3 text-sm">
          <Link
            ...
            className="rounded-md border border-red-800 px-3 py-2 text-red-100 hover:border-red-500"
          >
            New opportunity
          </Link>
          <Link
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="bg-black/70 border-red-900/40">
          ...
          <CardContent className="space-y-3 text-sm">
            {recent.length === 0 ? (
              <p className="text-red-200/70">No activity yet.</p>
            ) : (
              recent.map((item) => (
                <div key={item.id} className="flex items-center justify-between">
                  <span className="text-red-50">{item.title}</span>
        ...
        <Card className="bg-black/70 border-red-900/40">
          <CardHeader>
            <CardTitle className="text-red-100">Drafts</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            {drafts.length === 0 ? (
              <p className="text-red-200/70">No drafts yet.</p>
            ) : (
              drafts.map((item) => (
                <div key={item.id} className="flex items-center justify-between">
                  <span className="text-red-50">{item.title}</span>
        ...
        <Card className="bg-black/70 border-red-900/40">
          <CardHeader>
            <CardTitle className="text-red-100">Quick stats</CardTitle>
          </CardHeader>
